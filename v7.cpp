...
public:
    ...
    // В TaskManager
    void removeUserFromTask(int pid, int tid) {
    Project* p = findProject(pid);
    ...
    cout << "✔ Потребителят е премахнат от задачата.\n";
    }
...
int main() {
    ...
    User* u1 = manager.addUser("Maria", "maria@gmail.com");

manager.assignUserToTask(1, 2, u1);

manager.removeUserFromTask(1, 2);


    cout << "\n=== СРОКОВЕ ===";
    manager.showOverdue(1);
    manager.showDueSoon(1, 7);
    User* u9 = manager.addUser("Ivan", "ivan@gmail.com");
